...
// customPluginをファクトリ関数として定義
const createCustomPlugin = (cssFilepath: string): postcss.Plugin => {
	return {
			...
			root.walkAtRules('font-face', rule => {
				rule.walkDecls('src', decl => {
					...
					const newValues = urls?.map((url, index) => {
						const fontPath = path.join(dirname(cssFilepath), url);
...
export const createEmbededFontCss = async (cssFilePath: string, outputPath: string): Promise<string> => {
	...
	const customPlugin = createCustomPlugin(cssFilePath);
